...
import { Currency, currencyEquals, ETHER, Token } from '@hulkfinance/hulk-swap-sdk'
...
const mapWithoutUrls = (tokenMap: TokenAddressMap) =>
  Object.keys(tokenMap[defaultChainId]).reduce<{ [address: string]: Token }>((newMap, address) => {
    newMap[address] = tokenMap[defaultChainId][address].token
    return newMap
  }, {})
